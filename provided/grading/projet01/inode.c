...
void inode_print(const struct inode *inode)
{
    ...
    if(inode == NULL) {
        ...
        printf("i_size0: %u\n",inode->i_size0);
        printf("i_size1: %u\n",inode->i_size1);
        // correcteur: mauvais format (inode_getsize n'est pas unsigned)
        printf("size: %u\n",inode_getsize(inode));
    }
    printf("**********FS INODE END**********\n");
    fflush(stdout);
...
int inode_read(const struct unix_filesystem *u, uint16_t inr, struct inode *inode)
{
    ...
    // correcteur: inr >= 0 avec inr de type unsigned...
    ...
    // read sector
    struct inode inodes[INODES_PER_SECTOR];
    // correcteur: un peu compliqué juste pour une division entière
...
int inode_findsector(const struct unix_filesystem *u, const struct inode *i, int32_t file_sec_off)
{
    ...
    // correcteur: utilisez des define pour ces valeurs (magic numbers)
    ...
    if(size <= smallFileMaxSize) { // small file
        // correcteur: ce n'est pas le bon test, ceci ne fonctionne potentiellement pas
        // pour un fichier contenant 0 bytes de données mais ayant quand même un secteur de données
        ...
        // correcteur: même remarque sur ce test
        if(file_sec_off * SECTOR_SIZE >= size) { // invalid offset
            return ERR_OFFSET_OUT_OF_RANGE; // return approriate error code
        }
